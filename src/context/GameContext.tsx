...
function gameReducer(state: GameState, action: GameAction): GameState {
  switch (action.type) {
      ...
      if (correct) {
        const updatedPlayers = state.players.map((p, i) =>
          i === state.currentPlayerIndex
            ? { ...p, timeline: insertIntoTimeline(p.timeline, state.currentSong!, action.insertIndex) }
            : p
        );
        return { ...state, players: updatedPlayers, placementResult: "correct", isRevealed: true, phase: "feedback" };
      } else {
        const nextPlayerIdx = (state.currentPlayerIndex + 1) % state.players.length;
        return { ...state, placementResult: "wrong", isRevealed: true, phase: "stealing", stealingPlayerIndex: nextPlayerIdx };
      }
    }
    ...
    case "STEAL_CARD": {
      ...
      const correct = validatePlacement(stealer.timeline, state.currentSong, action.insertIndex);
      if (correct) {
        const updatedPlayers = state.players.map((p, i) =>
        ...
        return { ...state, players: updatedPlayers, phase: "feedback" };
      } else {
    ...
    case "NEXT_TURN": {
      ...
      if (winner) return { ...state, phase: "finished" };
      if (state.deck.length === 0) return { ...state, phase: "finished" };
    ...
    case "RESET_GAME": {
      return { ...initialState, players: state.players.map((p) => ({ ...p, timeline: [] })) };
    }
...
export const GameContext = createContext<{ state: GameState; dispatch: Dispatch<GameAction> }>({
  state: initialState,
  dispatch: () => null,
});
